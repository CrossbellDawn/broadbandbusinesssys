...
public class ZXingCodeEncodeUtil {
     ...
     * 生成二维码保存到D盘
     */
    public static void createZXingCodeSaveToDisk(String content, int width, int height,
    ...
    public static BufferedImage createZXingCodeNormal(String content, int width, int height) {
        try {
            BitMatrix encode = new MultiFormatWriter().encode(content, BarcodeFormat.QR_CODE, width,
                    height, hints);
            // 得到二维码的宽度
     ...
     * 
     */
    public static void createZxingCodeUseLogoSaveToDisk(String content, int width, int height,
            String savePath, String imageType, InputStream logoStream) {
        try {
     ...
     * 
     */
    public static BufferedImage createZxingCodeUseLogo(String content, int width, int height,
        ...
        try {
            ...
            if (null != codeNormal) {
                ...
                if (null != logoStream) {
                    ...
                    // 计算logo的可用宽高
                    int logo_width =
                            logo_max_width < old_logo_width ? logo_max_width : old_logo_width;
                    int logo_height =
                            logo_max_height < old_logo_height ? logo_max_height : old_logo_height;
                    ...
                    /**
                     * logoImage logo图片对象 x 开始画的x轴坐标 y 开始画的y轴的坐 logo_width 要画的x轴的长度 logo_height
                     * 要画的y车的长度 arg5 null
                     */
